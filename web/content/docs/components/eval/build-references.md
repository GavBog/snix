---
title: "Build references in derivations"
date: 2025-12-24T09:07:43+01:00
lastmod: 2025-12-24T09:07:43+01:00
draft: false
weight: 3
toc: true
---
This document describes how build references are calculated in Snix. Build
...
## Snix & string contexts

In the past, Snix did not track string contexts in its evaluator at all, see
the historical section for more information about that.

Snix tracks string contexts in every `NixString` structure and offers an API to
...
evaluation elsewhere in Snix, and scan each derivation attribute against this
...
Every time a Snix evaluation does something that causes a store interaction, a
...
Context-reading builtins can be implemented in Snix by adding `hasContext` and
...
value representation of Snix, however we should consider not doing this at all.
...
unnecessary in Snix. We should evaluate which other uses exist, and act on them
appropriately.

The initial danger with diverging here is that we might cause derivation hash
discrepancies between Snix and C++ Nix, which can make initial comparisons of
...
[3cd15c5b1f5a8e6de87d5b7e8cc2f1326b420c88](https://github.com/NixOS/nix/commit/3cd15c5b1f5a8e6de87d5b7e8cc2f1326b420c88).
...
[nix#4629](https://github.com/NixOS/nix/issues/4629).
...
[nixpkgs#211783](https://github.com/NixOS/nixpkgs/pull/211783) this is not true anymore.
...
the derivation during environmental check, but Snix would silently force remove
that reference.
